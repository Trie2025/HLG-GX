...
async function init() {
  ...
  checkUpdate(true); // 打开弹窗即强制刷新更新状态（跳过 6h 缓存）
  ...
  $('#crawl-btn').addEventListener('click', async () => {
    ...
    if (tab && tab.id != null) {
      chrome.tabs.sendMessage(tab.id, { type: 'crawlNow' }, () => {
        // 必须读取 lastError，否则非洛谷页面上会抛 Unchecked runtime.lastError
...
async function checkUpdate(force) {
  ...
  try {
    const info = await chrome.runtime.sendMessage({ type: 'checkUpdate', force: !!force });
